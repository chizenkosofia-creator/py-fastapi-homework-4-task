...
async def register_user(
    ...
    Args:
        ...
        background_tasks (BackgroundTasks): FastAPI background tasks runner, used to send
            the activation email without blocking the response.
        email_sender (EmailSenderInterface): The service used to send the activation email.
    ...
    try:
        ...
        background_tasks.add_task(
            email_sender.send_activation_email, str(user_data.email), activation_link
        )
...
async def activate_account(
    ...
    Args:
        ...
        background_tasks (BackgroundTasks): FastAPI background tasks runner, used to send
            the activation-complete email without blocking the response.
        email_sender (EmailSenderInterface): The service used to send the activation-complete email.
        db (AsyncSession): The asynchronous database session.
...
async def request_password_reset_token(
    ...
    Args:
        data (UserActivationRequestSchema): Contains the user's email and activation token.
        background_tasks (BackgroundTasks): FastAPI background tasks runner, used to send
            the activation-complete email without blocking the response.
        email_sender (EmailSenderInterface): The service used to send the activation-complete email.
        db (AsyncSession): The asynchronous database session.
